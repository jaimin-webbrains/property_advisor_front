...
const SubDistrict = (props) => {
  ...
  const subDistrict = useSelector(
    (store) => store.master.geolocation.subDistrict
  );
  ...
  useEffect(() => {
    ...
    return () => {
      dispatch({
        type: Geolocationconstants.DELETE_SUB_DISTRICT,
      });
    };
  }, []);
  ...
  const handleAddClick = (type) => {
    if (formik.values.name !== "") {
      if (type === "add") {
        dispatch(GeolocationActions.addOrUpdateSubDistrict(formik.values));
      } else if (type === "update") {
        dispatch(GeolocationActions.addOrUpdateSubDistrict(formik.values));
      }
    }
    if (type === "delete") {
      dispatch(
        GeolocationActions.deleteSubDistrict({ _id: formik.values._id })
      );
    }
    setModal(!modal);
  };
  const networkRoleConstants = [
    Geolocationconstants.GET_SUB_DISTRICT,
    Geolocationconstants.ADD_OR_UPDATE_SUB_DISTRICT,
    Geolocationconstants.DELETE_SUB_DISTRICT,
  ];
  return (
    <div className="container-fluid">
      <div className="row title-sec">
        <div className="col-sm headline">Subdistricts</div>
